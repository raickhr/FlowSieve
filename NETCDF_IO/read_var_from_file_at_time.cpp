 ...
 *  @param[in]      time_index          time index [logical, not dimensional] to load in
...
void read_var_from_file_at_time(
    ...
    #if DEBUG >= 1
    if (wRank == 0) {
        fprintf(stdout, "Attempting to read %s at time %'zu from %s\n", var_name.c_str(), time_index, buffer);
        fflush(stdout);
    }
    #endif
    ...
    for (int II = 0; II < num_dims; II++) {
        start[II] = 0;
        retval = nc_inq_dim(ncid, dim_ids[II], dim_name, &count[II]);
        if (retval != NC_NOERR ) { NC_ERR(retval, __LINE__, __FILE__); }
        if ( std::string(dim_name) == "time" ) {
            start[II] = time_index;
            count[II] = 1;
        }
        #if DEBUG >= 2
