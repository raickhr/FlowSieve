...
void apply_filter_at_point_aniso(
    ...
    std::vector<double> tmp_vals(Nfields, 0.),
                        ...
                        tmp_aniso_vals(Nfields * theta_bins, 0.),
                        kA_aniso_vals( Nfields * theta_bins, 0.);
    ...
    for (int LAT = LAT_lb; LAT < LAT_ub; LAT++) {
        ...
        for (int LON = LON_lb; LON < LON_ub; LON++ ) {
            ...
            double centre_lat = latitude.at( Ilat ),
                   centre_lon = longitude.at( Ilon ),
                   target_lat = latitude.at( curr_lat),
                   target_lon = longitude.at( curr_lon );
            double angle = atan2( sin( target_lon - centre_lon ) * cos( target_lat ),
                                  - cos( target_lon - centre_lon ) * cos( target_lat ) * sin( centre_lat ) 
                                  + sin( target_lat ) * cos( centre_lat )
                                );
            //int angle_bin = std::max(0, std::min( theta_bins-1, (int) floor( (M_PI + angle) / (2. * M_PI / theta_bins) )));
            int angle_bin = ( (int) floor( (M_PI + angle) / (2. * M_PI / theta_bins) ) ) % theta_bins;
            ...
            if ( is_water ) {
                for (size_t II = 0; II < Nfields; ++II) {
                    ...
                    if ( (curr_lon == Ilon) and (curr_lat == Ilat) ) {
                        // Distribute the centre point across all bins
                        for (int Ibin = 0; Ibin < theta_bins; Ibin++) {
                            tmp_aniso_vals[II*theta_bins + Ibin] += loc_val * loc_weight / theta_bins;
                            kA_aniso_vals[ II*theta_bins + Ibin] += loc_weight / theta_bins;
                        }
                    } else {
                        tmp_aniso_vals[II*theta_bins + angle_bin] += loc_val * loc_weight;
                        kA_aniso_vals[ II*theta_bins + angle_bin] += loc_weight;
                    }
                }
            }
        }
    ...
    //fprintf( stdout, "Applied the kernel, now gather stuff together.\n" );
    ...
    for (size_t II = 0; II < Nfields; ++II) {
        ...
        if (do_aniso) {
            // Get the local aniso value
            double sigma_p = 0, mu_p = 0, rms_p = 0, p_i = 0, w_i = 0, w_sum = 0,
                   sigma_d = 0, mu_d = 0, rms_d = 0;
            for (int Ibin = 0; Ibin < theta_bins; Ibin++) {
            ...
            for (int Ibin = 0; Ibin < theta_bins; Ibin++) {
                ...
                sigma_p += pow( p_i - mu_p, 2) * w_i;


                p_i = pow(tmp_aniso_vals[Ianiso] / kA_aniso_vals[Ianiso], 1);
                sigma_d += pow( p_i - mu_d, 2) * w_i;
            }
            sigma_p = sqrt( sigma_p / w_sum );
            sigma_d = sqrt( sigma_d / w_sum );

            //*(aniso_vals.at(II)) = ( (mu_p + rms_p) == 0 ) ? 0. : sigma_p / ( mu_p + rms_p );
            //*(aniso_vals_dim.at(II)) = ( (mu_d + rms_d) == 0 ) ? 0. : sigma_d / ( mu_d + rms_d );

            *(aniso_vals.at(II)) = ( (mu_d + rms_d) == 0 ) ? 0. : sigma_d / ( mu_d + rms_d );
            *(aniso_vals_dim.at(II)) = sigma_d;
        }
