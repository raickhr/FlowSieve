...
int main(int argc, char *argv[]) {
    ...
    std::vector< std::vector<double> > coarse_fields(Nvars), dl_coarse_fields(Nvars), dll_coarse_fields(Nvars), 
        aniso_fields(Nvars), aniso_dim_fields(Nvars);
    for (size_t field_ind = 0; field_ind < Nvars; field_ind++) {
        ...
        aniso_fields.at(field_ind).resize( Npts );
        aniso_dim_fields.at(field_ind).resize( Npts );
    }
    ...
    for ( Ivar = 0; Ivar < Nvars; Ivar++ ) {
        output_names.push_back( vars_to_filter.at(Ivar) );
        output_names.push_back( vars_to_filter.at(Ivar) + "_aniso" );
        output_names.push_back( vars_to_filter.at(Ivar) + "_aniso_dim" );
    }
    ...
    for (size_t ell_ind = 0; ell_ind < filter_scales.size(); ell_ind++) {
        ...
        if (not(constants::NO_FULL_OUTPUTS)) {
            ...
            initialize_output_file( source_data, output_names, fname, scale );
            ...
            for ( Ivar = 0; Ivar < Nvars; Ivar++ ) {
                write_field_to_output( aniso_fields.at(Ivar), vars_to_filter.at(Ivar) + "_aniso", starts, counts, fname, &source_data.mask );
                write_field_to_output( aniso_dim_fields.at(Ivar), vars_to_filter.at(Ivar) + "_aniso_dim", starts, counts, fname, &source_data.mask );
            }
        }
